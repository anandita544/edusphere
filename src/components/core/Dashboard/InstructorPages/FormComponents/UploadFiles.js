  ...
  return (
    <div className=" relative">
      <p className=" form-field-title">
        {label} {!viewData && <SupStar />}
      </p>
      <div
        className={` px-3 py-8 rounded-lg border-[1px] border-dashed border-richblack-600 bg-richblack-700`}
      >
        {previewSource ? (
          <div className=" flex flex-col w-full gap-4">
            {!video ? (
              <div className=" flex items-center flex-col gap-4">
                <div className="w-full">
                  <p className=" text-left text-richblack-200 text-sm">
                    Image Preview
                  </p>
                  <p className=" text-pink-400 text-xs">
                    This image preview will be shown across all devices
                  </p>
                </div>
                <img
                  src={previewSource}
            ...
            {!viewData && (
              <button
                onClick={() => cancelBtn()}
                className={`text-richblack-25 text-xs hover:underline ${
                  disabled ? "hidden" : ""
                }`}
              >
                Cancel
